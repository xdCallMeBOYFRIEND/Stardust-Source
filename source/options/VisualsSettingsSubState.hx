...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		var option:Option = new Option('Forever Combo Counter',
			'If checked, displays the combo counter with only the digits of your current combo.',
			'foreverCombo',
			BOOL);
		addOption(option);

		var option:Option = new Option('MFC Ranking',
			'If checked, changes SFC to be displayed as MFC.',
			'mfcRanking',
			BOOL);
		addOption(option);

		var option:Option = new Option('Hold Accuracy',
			"Increases accuracy if a sustain note is being held.",
			'holdAccuracy',
			BOOL);
		addOption(option);

		var option:Option = new Option('Longer Healthbar',
			"Self-explanatory.",
			'longerHealth',
			BOOL);
		addOption(option);
		...
		var option:Option = new Option('Opaque Sustains',
			"If checked, the sustain notes will be completely opaque like in current base game.",
			'opaqueSustains',
			BOOL);
		addOption(option);
