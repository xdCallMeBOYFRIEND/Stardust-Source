...
class FunkinLua {
	...
	public function new(scriptName:String, ?scriptType:String = "") {
		...
		Lua_helper.add_callback(lua, "changeLuaCharacter", function(tag:String, character:String){
			var char:Character = PlayState.instance.modchartCharacters.get(tag);
			if(char != null) makeLuaCharacter(tag, character, char.isPlayer);
		...
		Lua_helper.add_callback(lua,"stopIdle", function(id:String, bool:Bool) {
				...
				{
					PlayState.instance.modchartCharacters.get(id).skipDance = bool;
					return;
				}
				LuaUtils.getActorByName(id).skipDance = bool;
			});
	
			Lua_helper.add_callback(lua,"setDownscroll", function(id:Bool) {
				ClientPrefs.data.downScroll = id;
			});
		...
		Lua_helper.add_callback(lua, "makeCharacter", function(tag:String, character:String, isPlayer:Bool = false) { // just changing some names
			makeLuaCharacter(tag, character, isPlayer);
		});
	...
	public static function makeLuaCharacter(tag:String, character:String, isPlayer:Bool = false) {
		...
		var char:Character = PlayState.instance.modchartCharacters.get(tag);
		LuaUtils.getTargetInstance().add(char);

		if (position >= 0) //this should keep them in the same spot if they switch
		{
			LuaUtils.getTargetInstance().remove(char, true);
			LuaUtils.getTargetInstance().insert(position, char);
		}
		...
		if (!isPlayer)
		{
			if (charX == 0 && charY == 0) {
				...
				char.x = PlayState.instance.DAD_X + charX;
				char.y = PlayState.instance.DAD_Y + charY;
			}
		}
		else
		{
			if (charX == 0 && charY == 0) {
				...
				char.x = PlayState.instance.BF_X + charX;
				char.y = PlayState.instance.BF_Y + charY;
			}
		}
