  ...
  public function setCharacter(char:String):Void
  {
    ...
    switch (type){
      case LEGACY:
        ...
        loadGraphic(image); //Load stupidly first for getting the file size
        var widths = width;
        if (width == 450) {
            this.loadGraphic(image,true,Math.floor(width / 3), Math.floor(height));
            animation.add("idle",[0]);
            animation.add("confirm",[2]);
        } else {
            this.loadGraphic(image,true,Math.floor(width / 2), Math.floor(height));
            animation.add("idle",[0]);
