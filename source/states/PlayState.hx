...
class PlayState extends MusicBeatState
{
	...
	public var scoreGain:Bool = true;
	public var scoreSusGain:Bool = false;
	public var healthImage:String = "healthBar"; //Because y'know
	public var timeImage:String = "timeBar"; //Yeah why not
	...
	override public function create()
	{
		...
		timeBar = new Bar(0, timeTxt.y + (timeTxt.height / 4), timeImage, function() return songPercent, 0, 1);
		...
		if(ClientPrefs.data.timeBarType == 'Song Name' && ClientPrefs.data.timeBarType == 'Modern Time')
		{
			timeTxt.size = 24;
			timeTxt.y += 3;
		}
		...
		if (ClientPrefs.data.longerHealth) {
			healthImage = 'longhealthBar';
		}

		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), healthImage, function(){
		...
		if (ClientPrefs.data.playerBarColor) {
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			canPause = true;
			generateStaticArrows(0, !skipArrowStartTween, targetAlpha);
			generateStaticArrows(1, !skipArrowStartTween, targetAlpha);
	...
	public dynamic function fullComboFunction()
	{
		...
		if(songMisses == 0)
		{
			...
			if (ClientPrefs.data.mfcRanking) {
	...
	public var targetAlpha:Float = 1; //this too
	public var arrowStartY = 10;
	private function generateStaticArrows(player:Int, tweenShit:Bool = true, targetAlpha:Float = 1):Void
	{
		...
		for (i in 0...4)
		{
			...
			{
				//Because idk thought it'd look cool -- Future me here, I don't like this actually.
				/*if (ClientPrefs.data.downScroll) {
					babyArrow.y += arrowStartY;
				} else {
					babyArrow.y -= arrowStartY;
				}*/
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {/*y: babyArrow.y + arrowStartY,*/ alpha: targetAlpha}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i) / playbackRate});
			}
	...
	override public function update(elapsed:Float)
	{
		...
		else if (!paused && updateTime)
		{
			...
			if(ClientPrefs.data.timeBarType == 'Modern Time'){
				timeTxt.text = FlxStringUtil.formatTime(secondsTotal, false) + " / " + FlxStringUtil.formatTime(songLength / 1000, false);
			}
			if(ClientPrefs.data.timeBarType != 'Song Name' && ClientPrefs.data.timeBarType != 'Song Name + Time' && ClientPrefs.data.timeBarType != 'Modern Time'){
	...
	public var iconScaleTime:Float = 9;
...
////////////////////////////////////////
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (ClientPrefs.data.popUpRating)
		{
			...
			if (!PlayState.isPixelStage)
				{
					if (ratingEffects) {
						...
						comboSpr.scale.set(0.785, 0.785);

						FlxTween.tween(rating.scale, {x: 0.7, y: 0.7}, 0.5, {ease: FlxEase.expoOut});
						FlxTween.tween(comboSpr.scale, {x: 0.7, y: 0.7}, 0.5, {ease: FlxEase.expoOut});
						}
					if (ratingTilt) {
						rating.angularVelocity = FlxG.random.float(-15, 15);
						comboSpr.angularVelocity = FlxG.random.float(-15, 15);
						}
					}
				}
			...
			if (ClientPrefs.data.foreverCombo) {
				seperatedScore = Std.string(combo);
			} else {
				seperatedScore = Std.string(combo).lpad('0', 3);
			}
			for (i in 0...seperatedScore.length)
			{
				var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(uiPrefix + 'num' + Std.parseInt(seperatedScore.charAt(i)) + uiPostfix));
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.hitCausesMiss) //Common notes
		{
				...
				if(char != null)
				{
					...
					for (value in modchartCharacters.keys()) {
						var daLuaChar = modchartCharacters.get(value);
							if ((daLuaChar.isPlayer) || (!daLuaChar.isPlayer)) daLuaChar.holdTimer = 0;
					}
					// TODO: maybe move this all away into a seperate function
					if (noteRows[note.mustPress ? 0 : 1][note.row] != null && noteRows[note.mustPress ? 0 : 1][note.row].length > 1 && note.noteType != "Ghost Note" && ghostsAllowed)
					{
			...
			else if(!guitarHeroSustains && scoreSusGain){ //? Legacy scoring for sustains
				songScore += 125;
				updateScoreText();
			}
			//Fixed this because I kinda actually need it for smth
			var gainHealth:Bool = true; // prevent health gain, *if* sustains are treated as a singular note
			if (guitarHeroSustains && note.isSustainNote) gainHealth = false;
			if (gainHealth) health += note.hitHealth * healthGain;
			if (ClientPrefs.data.holdAccuracy) {
				RecalculateRating();
				totalNotesHit++;
				totalPlayed++;
			}
	...
	public function spawnOppNoteSplashOnNote(note:Note) {
		if(note != null) {
			var strum:StrumNote = opponentStrums.members[note.noteData];
			if(strum != null)
				spawnOppNoteSplash(note, strum);
		}
	}

	public function spawnOppNoteSplash(note:Note, strum:StrumNote) {
		var splash:NoteSplash = new NoteSplash();
		splash.babyArrow = strum;
		splash.spawnSplashNote(note);
		grpNoteSplashes.add(splash);
